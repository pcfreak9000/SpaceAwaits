...
public class RenderItemStrategy implements IRenderStrategy {
    ...
    public RenderItemStrategy(GameScreen renderer) {
        ...
        this.batch = this.render.getRenderHelper().getSpriteBatch();
    }
    ...
    public void render(Entity e, float dt) {
        ...
        if (stack != null && !stack.isEmpty()) {
            if (stack.getCount() == 1) {
                ...
                batch.draw(stack.getItem().getIcon().getRegion(), tc.position.x - mod, tc.position.y - mod,
                ...
                for (int i = 0; i < amount && i < offsets.length; i++) {
                    ...
                    batch.setColor(stack.getItem().getColor());
                    batch.draw(stack.getItem().getIcon().getRegion(), x - mod, y - mod, Item.WORLD_SIZE + mod * 2,
