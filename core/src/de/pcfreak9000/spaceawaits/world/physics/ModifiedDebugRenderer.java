...
import com.badlogic.gdx.box2d.utils.Box2dUtils;
...
public class ModifiedDebugRenderer implements Disposable {
	...
	public ModifiedDebugRenderer() {
		...
		b2DebugDraw = Box2d.b2DefaultDebugDraw();
	...
	public void render(b2WorldId world, Matrix4 projMatrix) {
		...
		renderer.begin();
		Box2d.b2World_Draw(world, b2DebugDraw.asPointer());
		renderer.end();
	}
	...
	private void drawSolidPolygon(b2Transform transform, b2Vec2.b2Vec2Pointer vertices, int vertexCount, float radius,
			b2HexColor color, VoidPointer context) {
		float[] verticesArray = new float[vertexCount * 2];
		...
		for (int i = 0; i < vertexCount - 2; i++) {
			renderer.triangle(verticesArray[0], verticesArray[1], verticesArray[i * 2 + 2], verticesArray[i * 2 + 3],
	...
	private void drawCircle(b2Vec2 center, float radius, b2HexColor color, VoidPointer context) {
		renderer.set(ShapeRenderer.ShapeType.Line);
		Box2dUtils.box2dColorToGDXColor(renderer.getColor(), color);
		renderer.circle(center.x(), center.y(), radius, Math.max(10, (int) (6 * (float) Math.cbrt(radius))));
	}
	...
	private void drawSolidCircle(b2Transform transform, float radius, b2HexColor color, VoidPointer context) {
		...
		Box2dUtils.box2dColorToGDXColor(renderer.getColor(), color);
		transform.p(TEMP_VEC_PTR);
		renderer.circle(TEMP_VEC_PTR.x(), TEMP_VEC_PTR.y(), radius, Math.max(10, (int)(6 * (float)Math.cbrt(radius))));
	}
	...
	private void drawTransform(b2Transform transform, VoidPointer context) {
		...
		renderer.line(px, py, px + axisScale * cos, py + axisScale * sin);

		renderer.setColor(Color.GREEN);
		renderer.line(px, py, px - axisScale * sin, py + axisScale * cos);
	}
