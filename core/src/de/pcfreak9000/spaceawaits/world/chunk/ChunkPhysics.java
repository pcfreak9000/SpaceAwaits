...
public class ChunkPhysics implements IBodyFactory {
	...
	private void createFixture(Tile tile, int gtx, int gty) { // This could be more memory efficient
		...
		if (!tile.hasCustomHitbox()) {
			...
			b2Vec2Pointer p = new b2Vec2Pointer(custom.length / 2, true);
			b2Vec2 tmp = new b2Vec2();
			for (int i = 0; i < custom.length / 2; i++) {
				tmp.x(METER_CONV.in(gtx - chunk.getGlobalTileX() + custom[2 * i] - 0.5f));
				tmp.y(METER_CONV.in(gty - chunk.getGlobalTileY() + custom[2 * i + 1] - 0.5f));
				p.set(tmp, i);
			}
			//could be precomputed. we would then use b2MakeOffsetPolygon to place the polygon at the correct place...
			b2Hull hull = Box2d.b2ComputeHull(p, custom.length / 2);
			shape = Box2d.b2MakePolygon(hull.asPointer(), 0);
		}
	...
	private final class ListenerClass implements ChunkChangeListener {
		...
		public void onTileStateChange(Chunk chunk, TileState state, Tile newTile, Tile oldTile, int gtx, int gty,
			...
			if (isSolidAndFilling(oldTile) != isSolidAndFilling(newTile)) {
				...
				// new and old are both solid and filling or both not solid and filling. The
				// fixture is retained if both are solid and filling.
				...
				} else if (!normalTileFilled) {// deal with custom hitboxes. In this case doesn't change the state of
												// surrounding fixtures, so this is relatively simple.
