...
public class TestWorldProvider implements WorldProvider {
    ...
    private Chunk requestRegion(int rx, int ry) {
        if (gen.getMeta().inChunkBounds(rx, ry)) {
            ...
            if (r == null) {
                ...
                this.chunks[rx][ry] = r;
                this.gen.getChunkGenerator().generateChunk(r, SpaceAwaits.getSpaceAwaits().getWorldManager().getWorldAccess());
            }
            return r;
        }
        return null;
    ...
    public Background getBackground() {
        return gen.getBackground();
    }

    @Override
    public AmbientLightProvider getAmbientLight() {
        return gen.getAmbientLight();
    }
